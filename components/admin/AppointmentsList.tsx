  ...
  const handleActionWithWA = async (
    ...
    if (result && result.whatsappUrl) {
      setWhatsappModal({
        ...
        action: action === 'confirm' ? 'Confirmed' : action === 'complete' ? 'Completed' : 'Rescheduled',
      });
    }
  };
                          ...
                          >
                            ...
                            <td className="py-3.5 px-4 text-right whitespace-nowrap">
                              <div className="flex items-center justify-end gap-1.5">
                                ...
                                {bk.status === 'pending' && (
                                  <>
                                    {bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? (
                                      <button
                                        onClick={async () => {
                                          try {
                                            const res = await onAction(bk.id, 'check-payment-db');
                                            if (res && (res as any).paid) {
                                              alert('Payment verified and updated in database! Slot is active.');
                                              onRefresh();
                                            } else {
                                              alert(`Payment check completed. Status: ${(res as any).paymentStatus || 'unpaid'}`);
                                            }
                                          } catch (err: any) {
                                            alert(err.message || 'Verification failed');
                                          }
                                        }}
                                        className="px-2.5 py-1 bg-[#1B4F72] hover:bg-[#0B1B29] text-white rounded-lg text-[10px] font-bold uppercase"
                                    ...
                                    <button
                                      ...
                                      className="px-2.5 py-1 bg-[#1B4F72] hover:bg-[#0B1B29] text-white rounded-lg text-[10px] font-bold uppercase"
                                      title={bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? "Resend Payment Link" : "Confirm Slot"}
                                    >
                                      {bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? 'Resend Link' : 'Confirm'}
                                    </button>
                                  </>
                                )}
                                ...
                                <button
                                  ...
                                  className={`p-1.5 rounded-lg border text-[11px] font-bold flex items-center gap-1 transition-all ${isExpanded ? 'bg-primary text-white border-primary' : 'bg-gray-50 text-gray-700 hover:bg-gray-100 border-gray-200'
                                    }`}
                                >
                                      ...
                                      <div className="space-y-1.5 text-xs">
                                        {bk.age && (
                                          <div className="flex justify-between items-center py-1 border-b border-gray-100">
                                            ...
                                            <span className="font-bold text-gray-900">{bk.age} Years</span>
                                          </div>
                                        )}
                                        {bk.gender && (
                                          <div className="flex justify-between items-center py-1 border-b border-gray-100">
                                            <span className="text-gray-500 font-medium">Gender</span>
                                            <span className="font-bold text-gray-900 capitalize">{bk.gender}</span>
                                          </div>
                                        )}
                                        {role === 'doctor' && bk.skinType && (
                                          <div className="flex justify-between items-center py-1 border-b border-gray-100">
                                            <span className="text-gray-500 font-medium">Skin Type</span>
                                            <span className="font-bold text-gray-900">{bk.skinType}</span>
                                          </div>
                                        )}
                                        {bk.address && (
                                          <div className="py-1.5">
                                            <span className="text-gray-500 font-medium block mb-0.5">Address</span>
                                            <span className="font-bold text-gray-900 block leading-relaxed">{bk.address}</span>
                                          </div>
                                        )}
                                      </div>

                                      {role === 'doctor' && (
                                        <div className="pt-2 border-t border-gray-250/60 space-y-2">
                                          {bk.problemDescription ? (
                                            <div className="bg-blue-50/20 border border-blue-100 p-2.5 rounded-lg">
                                              <span className="text-[9px] uppercase font-black tracking-widest text-primary block mb-1">Problem Condition</span>
                                              <p className="text-gray-700 leading-normal">{bk.problemDescription}</p>
                                            </div>
                                          ) : (
                                            <p className="text-gray-455 italic">No problem description submitted.</p>
                                          )}
                                          {bk.previousMedication && (
                                            <div className="bg-gray-50 border border-gray-200/60 p-2.5 rounded-lg">
                                              <span className="text-[9px] uppercase font-black tracking-widest text-gray-600 block mb-1">Previous Meds</span>
                                              <p className="text-gray-700 leading-normal">{bk.previousMedication}</p>
                                            </div>
                                          )}
                                        </div>
                                      )}
                                    ...
                                    <div className="bg-white p-3.5 rounded-xl border border-gray-200 space-y-2">
                                      ...
                                      {bk.bookingType === 'online' && bk.meetingLink && (
                                        <div className="bg-teal-50/20 border border-teal-100 p-2 rounded-xl text-[10px] space-y-1 text-gray-700 font-semibold leading-relaxed mt-2 text-left">
                                          ...
                                          <p>Meeting URL: <a href={bk.meetingLink?.replace('meet.jit.si', 'meet.ffmuc.net')} target="_blank" rel="noopener noreferrer" className="text-[#1B4F72] hover:underline font-bold">{bk.meetingLink?.replace('meet.jit.si', 'meet.ffmuc.net')}</a></p>
                                    ...
                                    <div className="bg-white p-3.5 rounded-xl border border-gray-200 space-y-2">
                                      ...
                                      <div className="flex flex-wrap gap-1.5">
                                        ...
                                        {bk.status === 'completed' ? (
                                          <button
                                            onClick={() => {
                                              const msg = `*💖 Skin Hub Clinic — Thank You for Visiting! 🙏*\n\n` +
                                                `Namaste *${bk.name}*! 🌸\n\n` +
                                                `Hum aasha karte hain ki aaj clinic mein aapka consultation/treatment experience achha raha hoga.\n\n` +
                                                `*📋 Session Summary:*\n` +
                                                `• *Booking ID:* #${bk.id}\n` +
                                                `• *Service:* ${bk.service}\n` +
                                                `• *Doctor:* Dr. Prateek Tiwari\n` +
                                                `${bk.nextScheduleDate ? `• *Next Follow-up:* ${bk.nextScheduleDate} 🗓️\n` : ''}\n` +
                                                `*🌸 Post-Care Tips:*\n` +
                                                ...
                                                `*⭐ Share Your Feedback:*\n` +
                                                `Aapka feedback humare liye bohot valuable hai. Please apna review zaroor share karein:\n` +
                                                `🔗 https://g.page/r/skinhubujjain/review\n\n` +
                                                `Aapki skin hamesha swasth aur glow karti rahe! ✨\n\n` +
                                                `Warm regards,\n` +
                                                `*Dr. Prateek Tiwari & Team*\n` +
                                                `*Skin Hub Clinic* 🙏`;
                                              const waUrl = `https://wa.me/${bk.phone.replace(/\D/g, '')}?text=${encodeURIComponent(msg)}`;
                                              window.open(waUrl, '_blank');
                                            }}
                                            className="px-3 py-1.5 bg-teal-600 hover:bg-teal-700 text-white rounded-lg text-[10px] font-bold flex items-center gap-1 cursor-pointer"
                                          >
                                          ...
                                          <button
                                            ...
                                            className="px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg text-[10px] font-bold flex items-center gap-1 cursor-pointer"
                                          >
                                        ...
                                        {bk.paymentStatus === 'paid' && (
                                          <button
                                            ...
                                            className="px-3 py-1.5 bg-gray-150 hover:bg-gray-200 text-gray-700 border rounded-lg text-[10px] font-bold flex items-center gap-1 cursor-pointer"
                                          >
                  ...
                  >
                            ...
                            <div className="flex flex-wrap gap-x-3 gap-y-1">
                              {bk.age && (
                                <span className="text-gray-600">
                                  <strong>Age:</strong> <span className="text-gray-900 font-bold">{bk.age}</span>
                                </span>
                              )}
                              {bk.gender && (
                                <span className="text-gray-600">
                                  <strong>Gender:</strong> <span className="text-gray-900 font-bold capitalize">{bk.gender}</span>
                                </span>
                              )}
                              {role === 'doctor' && bk.skinType && (
                                <span className="text-gray-600">
                                  <strong>Skin:</strong> <span className="text-gray-900 font-bold">{bk.skinType}</span>
                                </span>
                              )}
                            </div>
                            {bk.address && (
                              <div className="text-gray-600 border-t border-gray-100 pt-1.5 mt-1 leading-relaxed">
                                <strong>Address:</strong> <span className="text-gray-900 font-bold">{bk.address}</span>
                              </div>
                            )}
                          </div>
                        )}
                        {role === 'doctor' && bk.problemDescription && (
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-2 border-t">
                        ...
                        <>
                          {bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? (
                            <>
                              <button
                                onClick={async () => {
                                  if (confirm(`Verify payment for ${bk.name}?`)) {
                                    try {
                                      const res = await onAction(bk.id, 'verify-payment-link');
                                      if (res && (res as any).paid) {
                                        alert('Payment verified successfully! Slot confirmed.');
                                        onRefresh();
                                      } else {
                                        alert(`Payment not yet completed. Status: ${(res as any).paymentLinkStatus || 'unpaid'}`);
                                      }
                                    } catch (err: any) {
                                      alert(err.message || 'Verification failed');
                                    }
                                ...
                                className="px-3 py-1.5 bg-blue-650 hover:bg-blue-750 text-white rounded-lg text-[10px] font-bold uppercase"
                              ...
                              <button
                                onClick={async () => {
                                  if (confirm(`Mark online booking for ${bk.name} as Paid manually? This will confirm the appointment and generate meeting details.`)) {
                                    try {
                                      const res = await onAction(bk.id, 'mark-paid-manual');
                                      if (res && (res as any).paid) {
                                        alert('Payment marked as paid manually! Slot confirmed.');
                                        onRefresh();
                                      } else {
                                        alert('Action failed');
                                      }
                          ...
                          <button
                            onClick={() => handleActionWithWA(bk.id, 'confirm', undefined, undefined, undefined, undefined, bk.name)}
                            className="px-3 py-1.5 bg-[#1B4F72] hover:bg-[#0B1B29] text-white rounded-lg text-[10px] font-bold uppercase"
                            title={bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? "Resend Payment Link" : "Confirm Slot"}
                          >
                            {bk.bookingType === 'online' && bk.razorpayPaymentLinkId ? 'Resend Link' : 'Confirm'}
                          </button>
                      ...
                      {bk.paymentStatus === 'paid' && (
                        <button
                          onClick={() => window.open(`/api/appointments/invoice?id=${bk.id}`, '_blank')}
                          className="px-3 py-1.5 bg-gray-150 hover:bg-gray-200 text-gray-700 border rounded-lg text-[10px] font-bold uppercase cursor-pointer flex items-center gap-1"
                        >
