...
export async function POST(req: Request) {
  try {
    ...
    // 4. Create in-app notification for Staff & Doctor
    try {
      await createNotification(
        'booking_new',
        '🎥 New Online Video Consultation',
        `New consultation booking from ${appointment.name} (${appointment.phone}) for ${appointment.preferredDate} [${appointment.preferredTimeSlot}]`
      );
    } catch (notifErr) {
      console.error('Failed to create in-app notification for booking:', notifErr);
    }
