...
export async function POST(req: Request) {
  try {
    ...
    if (isMock || (!RAZORPAY_KEY_SECRET && razorpay_order_id.startsWith('mock_order_'))) {
      ...
      // Log notification for staff dashboard
      await createNotification(
        'payment_received',
        'Video Consultation Booked',
        `Patient ${appointment.name} booked a video consultation on ${appointment.preferredDate} at ${appointment.preferredTimeSlot}. Payment of Rs. ${appointment.amountPaid} verified (Mock).`
      );
    ...
    await createNotification(
      'payment_received',
      'Video Consultation Booked',
      `Patient ${appointment.name} booked a video consultation on ${appointment.preferredDate} at ${appointment.preferredTimeSlot}. Payment of Rs. ${appointment.amountPaid} verified (Razorpay).`
    );
